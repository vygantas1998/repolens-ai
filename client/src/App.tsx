...
export function App() {
  ...
  return (
    <main className="grid min-h-screen bg-[linear-gradient(135deg,#050712,#0b1020_48%,#050712)] lg:grid-cols-[86px_minmax(0,1fr)_370px]">
      <aside className="flex items-center justify-center border-b border-slate-800 bg-slate-950/60 p-4 backdrop-blur lg:min-h-screen lg:flex-col lg:justify-start lg:border-b-0 lg:border-r lg:pt-6">
      ...
      <section className="p-5 sm:p-7">
        ...
        <section className="grid gap-5 xl:grid-cols-[340px_minmax(0,1fr)]">
          ...
          {analysis ? (
            <ArchitectureMap analysis={analysis} selectedNodeId={selectedNodeId} onSelectNode={setSelectedNodeId} />
          ) : (
            <div className="relative flex min-h-[640px] items-center justify-center overflow-hidden rounded-[1.75rem] border border-dashed border-slate-700/70 bg-slate-950/40 p-8 text-center text-sm text-muted-foreground">
      ...
      {analysis ? (
        <ResultsPanel analysis={analysis} selectedNode={selectedNode} />
      ) : (
